...
filtered_data <- merged_data %>%
  ...
  filter(Date %in% filter_date, Genotype %in% c("N2", "cest-2.1", "cest-1.2"))  %>%
  mutate(Genotype = fct_relevel(Genotype, "N2", "cest-2.1", "cest-1.2"))
# Filter Data to Date
#filtered_data <- merged_data %>%
  #filter(Date %in% filter_date, Genotype %in% c("N2", "cest-2.1", "cest-1.2"))
# Reorder Genotype 

filtered_data$Genotype <- factor(filtered_data$Genotype, levels = c("N2","cest-2.1", "cest-1.2")) 
